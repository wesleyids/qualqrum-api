...
public class ValidatorResource {
    ...
    @Info(description = "Valida se a senha é valida", path = "/senha", params = { "config" })
    @GetMapping(value = "/senha")
    public ResponseEntity<?> senha(@RequestParam("value") String email,
            @RequestParam("config") String conterCaracteresX) {
        boolean valid = Validator.emailValid(email);
        return ResponseEntity.ok().body(valid);
    }
}
